...
CREATE TABLE IF NOT EXISTS Transactions(
    id serial NOT NULL PRIMARY KEY INCREMENT,
...
func NewDbHelper() (DBHelper, error) {
	db, err := sqlx.Connect("postgres", "user=bank dbname=bar sslmode=disable")
	if err != nil {
		return DBHelper{}, err
	}
	db.MustExec(schema)
...
func (dbHelper *DBHelper) GetUser(userName string) (models.User, error, bool) {
	user := models.User{}
	err := dbHelper.dbConnection.Select(&user, "SELECT * FROM Users WHERE user_name=$1", userName)
	if err != nil {
...
func (dbHelper *DBHelper) ExecuteTransaction(transaction models.Transaction) (bool, error) {
	...
	result, err = tx.Exec("INSERT INTO Transactions(sender_user_name,"+
